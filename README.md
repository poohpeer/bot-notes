...
(ссылки) + M4 (транскрипция): можно отправить боту текст, ссылку (обычную,
YouTube, короткую ссылку карт) или голосовое сообщение в личном чате,
переключить приватность кнопкой, найти заметку `/search`. Голосовые
заметки и посты Instagram транскрибируются через `faster-whisper` на
отдельном тяжёлом воркере (`notes-worker-heavy`), не блокируя сохранение
обычного текста. Неудачное извлечение не роняет заметку, индексируется
исходный текст (деградация, см. `03-ingest.md`). LLM-обогащение и группы -
