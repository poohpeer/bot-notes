...
(ссылки): можно отправить боту текст или ссылку (обычную, YouTube, короткую
ссылку карт) в личном чате, переключить приватность кнопкой, найти заметку
`/search`. Обычная ссылка индексируется через `trafilatura`, YouTube - через
заголовок/описание/субтитры, карта - через имя места из финального URL;
неудачное извлечение не роняет заметку, индексируется исходный текст
(деградация, см. `03-ingest.md`). Транскрипция, LLM-обогащение и группы -
