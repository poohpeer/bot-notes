...
"⏱ Обработка завершена. Заняло: N.Nс" плюс превью первых ~150 символов
реально проиндексированного текста - тем же способом, каким `smart_answer`
шлёт готовый ответ (`TelegramSender`, прямой вызов Bot API из воркера, не
через диспетчер бота). Время считается от начала экстракции до записи
чанков включительно - `enrich_note` (обогащение) в него не входит, он и так
асинхронный и не блокирует `status='done'`.

Превью - это `text` из `process_note`, тот же самый, что пошёл в `chunk()` и
эмбеддинг, а не свежая LLM-суммаризация: `notes.summary` на этот момент ещё
не существует (`enrich_note` ставится в очередь только что и выполнится
позже), да и не нужен - смысл превью в том, чтобы подтвердить, что
проиндексировался реальный контент, а не, например, голый URL из-за молча
упавшей экстракции.
