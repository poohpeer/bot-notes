...
| ai-proxy | Генеративный LLM (провайдер `codex`) | Внутренний `poohpeer/ai-proxy`, контракт зафиксирован | Деградация: заметки сохраняются без заголовка и тегов; `/smart_search` остаётся обычным поиском |
...
Бот не выполняет сетевых загрузок контента и не вызывает LLM синхронно. Всё
долгое уходит в очередь, включая синтез ответа `/smart_search` (задача
`smart_answer` в очереди `llm`): обычная выдача отдаётся сразу, умный ответ
досылается новым сообщением. Причина - провайдер ai-proxy `codex` работает
через CLI с таймаутами в минуты, несовместимыми с синхронным ожиданием в чате.
...
задачи обогащения и синтеза `/smart_search` через ai-proxy. Лёгкий образ без ffmpeg и torch-стека
для Whisper.
...
│   ├── clients/                        # embeddings, ai-proxy (LLMClient), telegram helpers
