...
- `TELEGRAM_BOT_TOKEN`, `DATABASE_URL`, `REDIS_URL` - в Secret, монтируются
  переменными окружения. `PROXY_AI_URL` - в ConfigMap: это внутренний адрес
  сервиса (`http://ai-proxy:8787`), не секрет. ai-proxy аутентификации не
  имеет и наружу не выставлен; бот и ai-proxy должны быть в одном неймспейсе
  (иначе - FQDN `http://ai-proxy.<namespace>.svc.cluster.local:8787`).
