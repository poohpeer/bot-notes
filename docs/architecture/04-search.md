...
NLLB-200-distilled-600M переводит и текст заметки (в `process_note_async`,
перед `embed_passages`), и поисковый запрос (в `run_search`, перед
`embed_query`) на общий целевой язык (`TRANSLATE_TARGET_LANG=eng_Latn`) -
так оба сравниваемых вектора считаются по тексту на одном языке независимо
от того, на каком языке они написаны на самом деле. `chunk_text` в БД (то,
что видит пользователь) остаётся исходным, непереведённым - переводу
подвергается только то, что уходит в эмбеддинг.

Best-effort в обоих местах: недоступный или упавший notes-translate не
