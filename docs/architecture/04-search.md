...
`max_distance` (`SEARCH_MAX_DISTANCE`, по умолчанию 0.25) - фаза 3 дополнительно
...
заметки пользователя. Первая прикидка была 0.5 и оказалась сильно занижена:
для коротких текстов заметок `intfloat/multilingual-e5-base` сжимает
distance в узкий диапазон независимо от темы - реальный запрос
"порекомендуй уличную еду в тбилиси" дал 0.145 к релевантному видео, но
всего 0.206-0.221 к не связанным заметкам про покупки. 0.25 - тоже не
откалиброванное значение, просто подогнанное под этот один реальный пример;
стоит пересмотреть, когда накопится достаточно реального трафика поиска.
