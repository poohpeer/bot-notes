...
`max_distance` (`SEARCH_MAX_DISTANCE`, по умолчанию 0.5) - фаза 3 дополнительно
...
заметки пользователя. 0.5 - грубая отправная точка для
`intfloat/multilingual-e5-base`, не откалиброванная эмпирически; стоит
пересмотреть, когда накопится достаточно реального трафика поиска.
