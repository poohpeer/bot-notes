...
`max_distance` (`SEARCH_MAX_DISTANCE`, по умолчанию 0.18) - фаза 3 дополнительно
...
заметки пользователя. 0.5, потом 0.25 - обе оказались занижены на одном и
том же реальном примере: для коротких текстов заметок
`intfloat/multilingual-e5-base` сжимает distance в узкий диапазон
независимо от темы - запрос "порекомендуй уличную еду в тбилиси" дал 0.145
к релевантному видео, но всего 0.206-0.221 к не связанным заметкам про
покупки. 0.18 попадает в этот ~0.06-широкий зазор. Абсолютный порог - грубый
инструмент для настолько сжатой модели; относительный порог (держать хиты в
пределах дельты от distance лучшего результата, а не от фиксированного
потолка) подстроился бы под это автоматически - стоит перейти на него, когда
накопится достаточно реального трафика поиска, вместо повторной ручной
подгонки этого числа по одному примеру.
