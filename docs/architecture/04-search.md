...
`structured["subjects"]` (список предметов, записанный `/events_table`
при создании) реально содержит слово из запроса (грубое совпадение по
первым 5 символам, не настоящая морфология - "математик" внутри
"математике"), а есть и такие, у которых нет ни одного совпадающего
предмета - вторые отбрасываются, первые остаются. Если совпадений нет ни
у одной заметки - фильтр не применяется вообще (нечем отличить одну от
другой, лучше широкий ответ, чем пустой). Другие `source_type` фильтр не
трогает.
