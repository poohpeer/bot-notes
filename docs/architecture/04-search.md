...
| `/smart_search <запрос>` | Переписывание запроса, затем векторный поиск, затем синтез ответа по найденному | да, синхронно |
...
```mermaid
flowchart TD
    A[Запрос пользователя] --> B[proxy-ai: переписывание запроса]
    B --> C[1-3 варианта формулировки]
    C --> D[embeddings: kind=query для каждого]
    D --> E[ANN по каждому вектору]
    E --> F[Слияние и дедуп по note_id, лучшее расстояние]
    F --> G[Top-N чанков в контекст]
    G --> H[proxy-ai: синтез ответа со ссылками на заметки]
    H --> I[Ответ + карточки источников]
```

Пользователь ждёт, поэтому:

- на время обработки шлётся `sendChatAction: typing` с обновлением;
- общий бюджет времени ограничен (порядка 30 секунд), при превышении
  отдаётся результат обычного `/search` с пометкой, что умный режим не успел;
- любая ошибка proxy-ai деградирует до обычного `/search`, а не до ошибки;
- ACL применяется на шаге E, до попадания текста в контекст LLM. Чужой
