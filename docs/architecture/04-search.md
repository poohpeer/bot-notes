...
Заметки в этом продукте часто на разных языках (в основном иврит и
русский), а вопрос может быть задан на третьем - `rag_answer.md` просит
LLM переводить только связующий текст, а сами факты (даты, названия
событий/предметов) передавать дословно, как в заметке, не переводя: живой
пример - ответ про расписание экзаменов на иврите вперемешку с русским
переводом внутри одной строки читался вразнобой (Telegram разворачивает
направление текста при таком смешении). Дословная цитата снимает и риск
исказить факт при переводе.

Ответ (`result.text`, экранированный) оборачивается в `<pre>...</pre>` -
моноширинный блок читается заметно лучше для перечислений, и заодно
нейтрализует markdown-`**bold**`, который LLM иногда вставляет по
привычке (`parse_mode="HTML"` его не отрисовывает - `**text**` просто
остаётся текстом с двумя звёздочками). Блок `Источники:` ниже - **не**
внутри `<pre>`: Telegram не разрешает другие сущности (в т.ч.
`render_places`'ные `<a href>` на Google Maps) внутри `<pre>`, поэтому
ссылки должны остаться снаружи.
