...
async def test_merge_table_event_subjects_uses_canonical_subjects_for_structured(db_session):
    """tags stays in the table's own language (display); structured gets
    canonical_subjects instead, when given - 04-search.md's subject filter
    reads structured, not tags."""
    repo = NoteRepository(db_session)
    note = await repo.create_note(
        user_id=1,
        chat_id=1,
    ...
    await repo.merge_table_event_subjects(
        note.id, subjects=["מתמטיקה"], canonical_subjects=["mathematics"]
    )
    await db_session.flush()

    refreshed = await repo.get(note.id)
    assert "מתמטיקה" in refreshed.tags
    assert refreshed.structured["subjects"] == ["mathematics"]
