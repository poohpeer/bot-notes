...
async def factory(db_engine):
    ...
    async def insert_done_note(*, user_id=1, chat_id=1, title=None, raw_text="hello") -> int:
...
async def test_llm_failure_sends_nothing_and_does_not_raise(factory):
    await factory.insert_done_note()
...
async def test_empty_synthesis_sends_nothing(factory):
    await factory.insert_done_note()
